...
# Controls
這邊僅列出可以操作設定的 options.properties，內部屬性請參考[properties內部屬性]區塊。
...
- target 帶入要建立的目標id，trigger 則帶入要渲染的選項`selectOptions`陣列
...
``` javascript let children = Dropdown.getDropdownChildren(); // 回傳
dropdownChildren 屬性 ```

<span className="sb-unstyled methods">.getTargetDropdown()</span>
``` javascript 
let target = Dropdown.getTargetDropdown(); // 回傳targetEl屬性(也就是`<ul>`)
```

<span className="sb-unstyled methods">.getBindSelect()</span>
``` javascript 
let select = Dropdown.getBindSelect(); // 回傳bindSelect屬性(也就是`<select>`)
```
