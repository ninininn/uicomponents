...
export const createTable = ({ id, name, limits, selection, maxWidth, elem, cols, customContent, tools, classes, complete, error, handler }) => {
    ...
    async function fetchData(url) {
        let data = await fetch(url).then(response => response.json()).then(json => {
            ...
            parent.appendChild(table_instance._elem);
        });
    }
